addLayer("sm", {
    ...
    update(delta) {
        ...
        player.sm.spinCost = player.sm.spinAmount.pow(1.5).add(1).mul(10000000)
        ...
        player.sm.chipsEffect[0] = player.sm.chips[0].add(1)
        player.sm.chipsEffect[1] = player.sm.chips[1].pow(0.825).add(1)
        player.sm.chipsEffect[2] = player.sm.chips[2].pow(0.75).add(1)
    ...
    evaluateRewards() {
        ...
        for (let i = 0; i < player.sm.slotIndexes.length; i++) {
            if (player.sm.slotIndexes[i] == 0) 
            {
                ...
                makeParticles(BIG_COOKIE_NUMBER, 1, `normal`, {x: 450 + i * 200, y: 500, text: "+" + format(player.sm.chipsToGet[0].mul(mult)) + " red chips."})
            }
            if (player.sm.slotIndexes[i] == 1) 
            {
                player.sm.chips[1] = player.sm.chips[1].add(player.sm.chipsToGet[1].mul(mult))
                makeParticles(BIG_COOKIE_NUMBER, 1, `normal`, {x: 450 + i * 200, y: 500, text: "+" + format(player.sm.chipsToGet[1].mul(mult)) + " blue chips."})
            }
            if (player.sm.slotIndexes[i] == 2) 
            {
                player.sm.chips[2] = player.sm.chips[2].add(player.sm.chipsToGet[2].mul(mult))
                makeParticles(BIG_COOKIE_NUMBER, 1, `normal`, {x: 450 + i * 200, y: 500, text: "+" + format(player.sm.chipsToGet[2].mul(mult)) + " yellow chips."})
            }
            if (player.sm.slotIndexes[i] == 3) 
            {
                player.cb.evolutionShards = player.cb.evolutionShards.add(Decimal.mul(6, mult))
                makeParticles(BIG_COOKIE_NUMBER, 1, `normal`, {x: 450 + i * 200, y: 500, text: "+" + formatWhole(Decimal.mul(6, mult)) + " evolution shards."})
            }
            if (player.sm.slotIndexes[i] == 4) 
            {
                player.cb.paragonShards = player.cb.paragonShards.add(Decimal.mul(3, mult))
                makeParticles(BIG_COOKIE_NUMBER, 1, `normal`, {x: 450 + i * 200, y: 500, text: "+" + formatWhole(Decimal.mul(3, mult)) + " paragon shards."})
            }
        }
    },
    spinSlots() {
    ...
    buyables: {
        ...
        102: {
            ...
            effect(x) { return Decimal.div(1, getBuyableAmount(this.layer, this.id).pow(0.45).mul(0.25).add(1)) },
            unlocked() { return true },
            cost(x) { return this.costGrowth().pow(x || getBuyableAmount(this.layer, this.id)).mul(this.costBase()).floor() },
            canAfford() { return this.currency().gte(this.cost()) },
            display() {
        ...
        stuff: {
            ...
            "Research": {
                ...
                content: [
                    ...
                    ], {width: "775px", height: "650px", backgroundColor: "#4a4a4a80", border: "3px solid white", borderRadius: "15px 0 0 15px"}],
                ]
            },
        },
    },
